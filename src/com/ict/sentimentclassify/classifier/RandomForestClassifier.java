...
public class RandomForestClassifier extends Classifier {
	static Logger logger = Logger.getLogger(RandomForestClassifier.class
			.getName());

	RandomForest randomForestClassifier;

	public RandomForestClassifier(String classifierName, String trainFileName) {
		super(classifierName, trainFileName);
		try {
			this.randomForestClassifier = new RandomForest();
			this.randomForestClassifier.buildClassifier(this.trainInstances);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ClassifyResultEvaluation classfy(String testFileName) {
		...
		for (int i = 0; i < classifyResultEvaluation.getNumTestData(); i++) {
			...
			try {
				classifyResultLabel = this.randomForestClassifier
						.classifyInstance(testInstances.instance(i));
				classifyResultDistribute = this.randomForestClassifier
